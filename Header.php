...
$(document).ready(function(){
    ...
    // $("#site-year-list").change(function(){
    //     $("#site-make-list option").show();
    //     $("#site-make-list option:first").hide();
    //     $(".class-button-visble-find").hide();
    // });

    // $("#site-make-list").change(function(){
    //     $("#site-model-list option").show();
    //     $("#site-model-list option:first").show();
    ...
    <div class="formstable">
      ...
      <div class="col-md-2">
        ...
        <select name="Models" id="site-model-list" class="form-select">
          ...
          $modelsQuery = "SELECT DISTINCT `Model` FROM `cars` "; 
          $modelsResult = mysqli_query($connection, $modelsQuery);

          if ($modelsResult) {
            while ($modelRow = mysqli_fetch_assoc($modelsResult)) {
              ?>
              <option value="<?php echo $modelRow['Model']; ?>"><?php echo $modelRow['Model']; ?></option>
              <?php
            }
            mysqli_free_result($modelsResult);
          } else {
            echo "Error: " . mysqli_error($connection);
          }
          ?>
